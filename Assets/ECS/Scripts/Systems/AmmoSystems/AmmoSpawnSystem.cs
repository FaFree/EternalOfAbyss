...
{
    public class AmmoSpawnSystem : UpdateSystem
    {
        private Event<AmmoRequest> ammoRequest;

        private GameObject turretAmmoPrefab;
        
        private GameObject ammoPrefab;

        private PullObject ammoPull;

        private PullObject turretAmmoPull;
        ...
        public override void OnAwake()
        {
            ...
            var turretAmmoKey = WorldModels.Default.Get<Prefabs>().prefabMap[WorldModels.Default.Get<TurretStatConfig>().ammoKey];
            
            this.ammoPrefab = Addressables.LoadAssetAsync<GameObject>(ammoKey).WaitForCompletion();
            this.turretAmmoPrefab = Addressables.LoadAssetAsync<GameObject>(turretAmmoKey).WaitForCompletion();
            
            GameObject ammoRoot = new GameObject("AmmoRoot");

            this.ammoPull = new PullObject(ammoPrefab, ammoRoot.transform, 10);
            this.turretAmmoPull = new PullObject(turretAmmoPrefab, ammoRoot.transform, 10);

        }

        public override void OnUpdate(float deltaTime)
        {
            if (!ammoRequest.IsPublished)
                return;

            var boostModel = WorldModels.Default.Get<BoostsModel>();
            
            foreach (var evt in ammoRequest.BatchedChanges)
            {
            ...
            bool isTripleArrow, bool isRebound, bool isPassing, bool isPlayer, bool isAutoArrow, EntityId entityId)
        {
            ...
            if (isPlayer)
                go = this.ammoPull.GetFreeElement();
            else
                go = this.turretAmmoPull.GetFreeElement();
            ...
            Vector3 direction, bool isRebound, Transform transform, bool isPassing, bool isPlayer)
        {
            if (isPlayer)
            {
                entity.SetComponent(new AmmoComponent
                {
